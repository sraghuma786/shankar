...
namespace SAF
{
    ...
    namespace shankarAzureFunctions

    {
        public static class TaskListFunction
        {
                ...
                [HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = "task")] HttpRequest req, ILogger log)
            {
                ...
                try
                {
                    using (NpgsqlConnection connection = new NpgsqlConnection(Environment.GetEnvironmentVariable("NpgSqlConnectionString")))
                    {
                        connection.Open();
                        if (!String.IsNullOrEmpty(input.Description))
                        {
                            var query = $"INSERT INTO TaskList (Id,Description,CreatedOn,IsDone) VALUES({input.Id},'{input.Description}', '{input.CreatedOn}' , '{false}')";
